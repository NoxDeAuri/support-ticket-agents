...
| Keyword-extraction model returns multiple comma-joined phrases (e.g. `"password reset, expired link"`) instead of one short phrase | Found immediately after fixing the above — a real ticket that should have matched `TCK-0012` returned zero results, because the search did one literal substring match on the *entire* string, comma included. Fixed in `data/tools.py`: keywords are split on commas and OR-matched as separate terms, so retrieval is robust to a model not strictly following the "2-4 words" instruction, instead of silently returning nothing |
